...
pub fn register_request_metrics(registry: &prometheus::Registry) -> RequestMetrics {
    let up = prometheus::IntGauge::new("svc_streaming_up", "1 if the process is running")
        .expect("valid metric definition");
    registry
        .register(Box::new(up.clone()))
        .expect("register svc_streaming_up");
    up.set(1);

    let http_requests_total = prometheus::IntCounterVec::new(
        prometheus::Opts::new(
            "svc_streaming_http_requests_total",
    ...
    registry
        ...
        .expect("register svc_streaming_http_requests_total");

    let http_request_duration_seconds = prometheus::HistogramVec::new(
        prometheus::HistogramOpts::new(
            "svc_streaming_http_request_duration_seconds",
    ...
    registry
        ...
        .expect("register svc_streaming_http_request_duration_seconds");
...
mod tests {
    ...
    fn resource_defaults_to_provided_service_name() {
        ...
        let res = resource("svc-streaming-test");
        let value = res.get(&opentelemetry::Key::from_static_str("service.name"));
        assert_eq!(
            value.map(|v| v.to_string()),
            Some("svc-streaming-test".to_string())
        );
    }
}
